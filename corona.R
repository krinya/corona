...
combinetreetable <- function(){
  cases_t <- fromwidetolong("https://github.com/CSSEGISandData/COVID-19/raw/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Confirmed.csv",
                        "cases")
  death_t <- fromwidetolong("https://github.com/CSSEGISandData/COVID-19/raw/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Deaths.csv",
                        "death")
  recovered_t <- fromwidetolong("https://github.com/CSSEGISandData/COVID-19/raw/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Recovered.csv",
                            coltoname = "recovered")
  data <- merge(cases_t, death_t[, .(`Province/State`, `Country/Region`, date, death)], by = c("Province/State", "Country/Region", "date"), all.x = T)
  data <- merge(data, recovered_t[, .(`Province/State`, `Country/Region`, date, recovered)], by = c("Province/State", "Country/Region", "date"), all.x = T)
...
countries <- c("France", "Japan", "Germany", "Italy", "Hungary", "Netherlands", "India")
